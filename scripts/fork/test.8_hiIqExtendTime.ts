...
async function extendHiIqLockTime() {
  ...
  const lockTime =
    Math.round(new Date().getTime() / 1000) + secondsInADay * 360;
  ...
  await runwithImpersonation(toAddress, provider, hre, async (signer: any) => {
    ...
    const gasCost2 = await hiiqContract.estimateGas.increase_unlock_time(
      lockTime
    );
    await (
      await hiiqContract.increase_unlock_time(lockTime, {gasLimit: gasCost2})
    ).wait();
