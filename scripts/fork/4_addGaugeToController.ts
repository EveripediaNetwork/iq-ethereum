...
async function addGaugeToController() {
  ...
  const gaugeABI = require('../../artifacts/src/Curve/HIIQGaugeController.vy/HIIQGaugeController')
    .abi;
  const rewardsDistABI = require('../../artifacts/src/Curve/GaugeRewardsDistributor.sol/GaugeRewardsDistributor')
    .abi;
  ...
  await runwithImpersonation(OWNER_ADDR, provider, hre, async (signer: any) => {
    ...
    const rewardsDistributor = new hre.ethers.Contract(
      REWARDS_DIST_ADDR,
      rewardsDistABI,
      signer
    );
    ...
    estGas = await gauge.estimateGas.change_gauge_weight(
      UNI_GAUGE_FRAX_IQ_ADDR,
      50
    );
    await gauge.change_gauge_weight(UNI_GAUGE_FRAX_IQ_ADDR, 50, {
      gasLimit: estGas,
    });
    ...
    estGas = await rewardsDistributor.estimateGas.setGaugeState(
      UNI_GAUGE_FRAX_IQ_ADDR,
      false,
      true
    );
    ...
    await rewardsDistributor.setGaugeState(UNI_GAUGE_ETH_IQ_ADDR, false, true, {
      gasLimit: estGas,
    });
