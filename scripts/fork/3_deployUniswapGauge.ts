...
async function deployUniswapGauge() {
  ...
  await runwithImpersonation(OWNER_ADDR, provider, hre, async (signer: any) => {
    ...
    const StakingRewardsMultiGauge = await hre.ethers.getContractFactory(
      'StakingRewardsMultiGauge',
      signer
    );
    console.log(
      'StakingRewardsMultiGauge.signer',
      StakingRewardsMultiGauge.signer.address
    ); //, signer)
    ...
    console.log(
      'stakingRewardsMultiGauge deployed to:',
      stakingRewardsMultiGauge.address
    );
  });
}
