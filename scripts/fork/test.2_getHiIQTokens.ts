...
async function getHiIQTokens() {
  ...
  await runwithImpersonation(IQOwner, provider, hre, async (signer: any) => {
    ...
    const tx = await iqContract.mint(toAddress, howManyTokens, {
      gasLimit: estGas,
    });
    ...
    console.log(
      ...
      hre.ethers.utils.formatUnits(await iqContract.balanceOf(toAddress), 18)
    );
  });
  ...
  await runwithImpersonation(toAddress, provider, hre, async (signer: any) => {
    ...
    const lockedAmount = hre.ethers.BigNumber.from(
      hre.ethers.utils.parseEther('100000')
    ); // 100K

    console.log('approve transfer for IQ');
    const gasCost1 = await iqContract.estimateGas.approve(
      hiiqAddress,
      lockedAmount
    );
    console.log('gasCost1', hre.ethers.utils.formatEther(gasCost1));
    await (
      await iqContract.approve(hiiqAddress, lockedAmount, {gasLimit: gasCost1})
    ).wait();

    console.log('create lock, get hiIQ');
    const gasCost2 = await hiiqContract.estimateGas.create_lock(
      lockedAmount,
      lockTime
    );
    await (
      await hiiqContract.create_lock(lockedAmount, lockTime, {
        gasLimit: gasCost2,
      })
    ).wait();
