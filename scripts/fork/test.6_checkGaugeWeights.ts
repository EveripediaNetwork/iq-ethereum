...
async function voteOnGauges() {
  ...
  await runwithImpersonation(testUser, provider, hre, async (signer: any) => {
    ...
    const iq_frax_gauge_weight = await gaugeController.get_gauge_weight(
      UNI_GAUGE_FRAX_IQ_ADDR,
      {gasLimit: 400000}
    );
    const iq_eth_gauge_weight = await gaugeController.get_gauge_weight(
      UNI_GAUGE_ETH_IQ_ADDR,
      {gasLimit: 400000}
    );
    const total_gauge_weight = await gaugeController.get_total_weight({
      gasLimit: 400000,
    });
