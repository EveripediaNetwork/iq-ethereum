...
def _checkpoint(addr: address, old_locked: LockedBalance, new_locked: LockedBalance):
    ...
    last_point: Point = Point({bias: 0, slope: 0, ts: block.timestamp, blk: block.number, iq_amt: 0})
    if _epoch > 0:
        last_point = self.point_history[_epoch]
    else:
        last_point.iq_amt = ERC20(self.token).balanceOf(self) # saves gas by only calling once
    ...
    for i in range(255):
        ...
        if t_i == block.timestamp:
            ...
            last_point.iq_amt = ERC20(self.token).balanceOf(self)
            break
        else:
    ...
    if addr != ZERO_ADDRESS:
        ...
        u_new.iq_amt = convert(self.locked[addr].amount, uint256)
...
def balanceOf(addr: address, _t: uint256 = block.timestamp) -> uint256:
    ...
    if _epoch == 0:
        ...
        weighted_supply: uint256 = last_point.iq_amt + (VOTE_WEIGHT_MULTIPLIER * unweighted_supply)
...
def balanceOfAt(addr: address, _block: uint256) -> uint256:
    ...
    weighted_supply: uint256 = upoint.iq_amt + (VOTE_WEIGHT_MULTIPLIER * unweighted_supply)

    if ((upoint.bias >= 0) or (upoint.iq_amt >= 0)):
        return weighted_supply
    else:
        return 0
