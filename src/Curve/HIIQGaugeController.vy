...
# HIIQ is used instead of veCRV.

# Everipedia Reviewer(s) / Contributor(s)
...
def __init__(_token: address, _voting_escrow: address):
    ...
    @param _token `ERC-20 IQ` contract address
...
def change_global_emission_rate(new_rate: uint256):
    """
    @notice Change IQ emission rate
    @param new_rate new emission rate (IQ per second)
    """
    assert msg.sender == self.admin
